...
that  primarily depends on how the payoff on the asset  correlates with the fluctuations in government's net-of-interest deficits. In particular, if the  payoff  is  high when the government needs revenue to finance a higher deficit, optimal hedging requires the government to issue positive debt, and 
...
\begin{equation}
U_{c,0}^{i}\left[ c_{i,0}-c_{1,0}\right] +x_{i,0}+\left( {U_{l,0}^{i}} l_{i,0}-U_{c,0}^{i}\frac{U_{l,0}^{1}}{U_{c,0}^{1}}l_{1,0}\right) = U_{c,0}^{i}p_0\tilde{b}_{i,-1} \text{ for all } i\geq 2
\end{equation}
...
Let $V(B\_)$ be the ex-ante value of  a Ramsey plan starting with initial government assets $B\_$.\footnote{As a consequence of quasi linearity, the Ramsey plan is time consistent and we can characterize it by a single Bellman equation instead of two as in  \ref{Sec: Bellman equations}} It satisfies the Bellman equation 
...
\subsection{Numerical appendix}\label{appndx: numerical appendix}

The appendix describes how we apply methods developed in \cite{Evans2014} to compute the solution of the N type agents in section \ref{sec: numerical results}. In order to focus on the essential steps we begin with slightly special case where $\log \theta_{i,t}=\log \overline{\theta}_i+\sigma_{\epsilon} \epsilon_t$, where $\epsilon_t$ is i.i.d with mean zero and variance one.\footnote{Extending the method to more general productivity process as in \eqref{wage equation calibration} is straightforward.}
...
\item A vector of individual control variables $y_{i,t} \in \mathcal{R}^{n_y}$. This will include choice for $z_{i,t}$
\item A vector of aggregate choice variables $Y_t \in \mathcal{R}^{n_Y}$
\item A distribution $\Gamma_{t-1}$ over $\in \mathcal{R}^{n_z}$. This can a measure with $N$ mass points.

\end{itemize}



We first stack all the conditions characterizing the Ramsey plan in the following categories

\underline{Individual constraints}
	\begin{equation}
	\label{individual const}
			\EE_{t-1}F(y_{i,t}, \EE_{t-1} y_{i,t}, Y_t, y_{i,t+1} ,\epsilon_t| z_{i,t-1},\sigma_\epsilon) = 0  
	\end{equation}
	which must hold for almost all $z_{i,t-1}$ in $\Gamma_{t-1}$
	
	\vspace{0.5cm}
	\underline{Aggregate constraints}
	\begin{equation}
	\label{aggregate const}
		\int G(y_{i,t},Y,\epsilon_t|z_{i,t-1},\sigma_\epsilon) d\Gamma_{t-1}(z_{i,t-1})= 0	
	\end{equation}
	
The solution to this \eqref{individual const} and \eqref{aggregate const} will be functions $y,Y,\Gamma$ s.t $y_{i,t}=y(\epsilon_t|z_{i,t},\Gamma_{t-1},\sigma_{\epsilon})$, $Y_{t}=y(\epsilon_t|\Gamma_{t-1},\sigma_{\epsilon})$ and $\Gamma_{t}=\Gamma(\epsilon_t|\Gamma_{t-1},\epsilon_t)$ respectively.\footnote {The term $E_{t-1}y_{i,t}$ in equation \ref{individual const} is a stand in to capture that a subset of individual controls can be $t-1$ measurable. }. For the calculations in section \ref{sec: numerical results} we construct an approximation $\{\hat{y}_t,\hat{Y}_t,\hat{\Gamma}_t\}_t $ for a given sequence of aggregate shocks $\epsilon^t$ and some initial $\Gamma_{-1}$.


The method involves an iterative process and steps are briefly described below. The details of actual implementation are elaborated next 


\begin{enumerate}
\item Given some $\Gamma_{t-1}$, compute the individual and aggregate policies in the limiting economy with $\sigma_{\epsilon}=0$. The choice of state variables ensures that in this limiting economy $\Gamma_{t}=\Gamma_{t-1}$.\footnote{The extension to more general environments where $\Gamma_t$ follows a deterministic path in the non stochastic limit can be found in \cite{Evans2014}} This involves solving the following set of equations:
\begin{equation}
	\label{individual const ss}
			F(\bar y_{i}, \bar{y}_i, \bar{Y}, \bar{y} ,0| \bar{z}_{i,t-1},0) = 0  
	\end{equation}	
	\begin{equation}
	\label{aggregate const ss}
		\int G(\bar{y},\bar{Y},0|z_{i,t-1},\sigma_\epsilon) d\Gamma_{t-1}(z_{i,t-1})= 0	
	\end{equation}
	
This steady state $\bar{y},\bar{Y}$ depends on $\Gamma_{t-1}$ has an interpretation that it recovers a complete markets economy (as the variance of the shocks go to zero, a risk free bond is trivially sufficient for complete spanning) with initial conditions $\Gamma_{t-1}$. 

	
	\item Next construct a truncated Taylor approximation to the individual and aggregate policy rules around the steady state obtained in the previous step, $\bar{y}(\Gamma_{t-1})$ and $Y(\Gamma_{t-1})$ that yields us an approximation
	
\begin{align}
y(\epsilon_t|z_{i,t-1},\Gamma_{i,t-1},\sigma_\epsilon) &\approx \frac{\partial y}{\partial \epsilon}(0|z_{i,t-1},\Gamma_{t-1},0) \epsilon_t\\
					&+  \frac{\partial^2 y}{\partial \epsilon^2}(0|z_{i,t-1},\Gamma_{t-1},0)\epsilon_t^2\\
					&+ \frac{\partial^2 y}{\partial \sigma_{\epsilon}^2}(0|z_{i,t-1},\Gamma_{t-1},0) \sigma_{\epsilon}^2
\end{align}and aggregate decisions rules 
\begin{align}
Y(\epsilon_t|\Gamma_{t-1},\sigma_{\epsilon}) &\approx \frac{\partial Y}{\partial \epsilon}(0|\Gamma_{t-1},0) \epsilon_t\\			
	&+  \frac{\partial^2 Y}{\partial \epsilon_t^2}(0|\Gamma_{t-1},0)\epsilon^2_t\\
					&+ \frac{\partial^2 Y}{\partial \sigma_{\epsilon}^2}(0|\Gamma_{t-1},0)\sigma_{\epsilon}^2
\end{align}	
	
This involves computing derivatives that are evaluated at the steady state that satisfy the total differentiation of system \eqref{individual const} and \eqref{aggregate const} at the non stochastic steady state associated with $\Gamma_{t-1}$. \footnote{For large $N$,calculating these derivatives can be further simplified for a class of problems where $\frac{\partial z_{i,t}}{\partial z_{i,t-1}}(0|z_{i,t-1},\Gamma_{t-1},0)$ is independent of $z_{i,t-1}$. In our context this turns out to be an identity matrix.}

\item Draw shocks $\epsilon_t$ and use the approximated policies in the previous states for each agent to obtain $z_{i,t}$ and hence $\Gamma_t$. 

\item Repeat step 1

\end{enumerate}

Note that the point of approximation and hence the derivatives that capture how agents respond to aggregate shocks changes along the history. This property is particularly attractive for problems where the convergence to the ergodic distribution is slow and mean of ergodic steady distribution can be sufficiently far away from the initial conditions. Since the method is modification of a perturbation approach, handling a high dimension $z_{i,t-1}$ is tractable as against other methods that solve for the policy rules using projection methods with basis of finite order polynomials.



In the last part we describe the choices of $y_{i},Y,z_{i}$ and $F,G$ for our problem. There are two choices we make that are worth noting: firstly we express the problem \eqref{eq:BM2} in section \ref{Sec: Bellman equations} in a slightly different manner. Secondly the solutions to FOC $F,G$ can be represented in a recursive manner with multiple choices for what constitutes the state variables. We make choices that turn out to be convenient for applying the pertubation method.

The Bellman equation from $t\geq 1$ described in section \ref{Sec: Bellman equations} in a modified as follows:

Let $x\_=\frac{U^i_c\_}{b_{i}}$ and $m^i\_\propto U^i_{c}$ with $\sum_{i}m^i=1$. Note that Ricardian equivalence implies that we can normalize $\sum_{i}\frac{x^i\_}{U^i_c\_}=0$. Thus the essential dimension of the state variables that describe the alternative Bellman equation is also $2N-2$ as in problem \eqref{eq:BM2} in section \ref{Sec: Bellman equations}. The reason we do this is to exploit the symmetry across agents that is useful in computing the derivatives. 


We denote the aggregate shock by $a(s)$ that maps to $\epsilon$ with the identity $a=1+\sigma_{\epsilon}\epsilon$.
...
\phi^i(s):&&	U^i_c(s)a(s)\theta^i(1-\tau_l(s)) = -U^i_l(s)\\
...
Now we describe our second choice of what constitutes $z_{i}$. The FOC's of the problem described above can be expressed as 
	...
	\end{itemize}
We implement the second one because when  $\sigma_{\epsilon}=0$, it is possbile for $N>2$ that there are multiple $\mu\_$ associated with an arbitrary $x\_$ that solve the FOC but technically only one survives if one takes a limit $\sigma_{\epsilon}\to 0$.  However flipping the state to be $\mu\_$ retrieves a unique $x\_$.\footnote{We solved in both ways (using projection methods) for $N = 2$ to verify that they produce same allocation.}. We augment the FOC with constraints that impose a subset of the controls $( x_-, \psi_-,\gamma_-)$ are independent of $s$.

Collecting all this gives us 
\begin{itemize}
\item Individual states: $z^i= (m^i,\mu^i)$
\item The distribution:  $\Gamma$ over $z= (m,\mu)\in \mathbb R^2$ with $N$ mass points.
\item Individual controls: $y = (m,\mu,c,l,\phi,\rho,x_-,\psi_-)$ and
\item Aggregate controls:		$Y = (\tau_l,T,\alpha,\xi,\gamma_-)$
\end{itemize}
