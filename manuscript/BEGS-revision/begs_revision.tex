...
The Ricardian equivlance theorem  seems to send a message that the level of government assets do not matter for welfare. However, a correct interpretation of the result in Theorem \ref{theorem: main} is that public debt and transfers are not separately pinned down by a competitive equilibrium allocation. As such, throughout the paper we will impose a normalization on  asset profiles: we assume that productivities are ordered as  $\theta_{1,t}\geq\theta_{2,t}\ldots\geq \theta_{I,t}$ and set $b_{I,t}=0$. This normalization allows us to interpret $-B_t=\sum_{i<I}n_ib_{i,t}$ as ``public debt''.  We justify this normalization by recovering a canonical representative agent economy with linear taxes as in \cite{Chari1994}, \cite{Aiyagari2002} when Pareto weights are such that the government does not care enough about redistribution. \footnote{This is explained more formally in a setting with two types of agents with preferences that are linear in consumption in section \ref{Sec: quasilinear}}. Limits on $\sum_{i<I}n_ib_{i,t}$ will be counterparts  to  debt limits in a representative agent economy. 
...
We will proceed our characterization of the Ramsey plan in two parts. First we consider the case when the Pareto weight of the productive agent is higher than a threshold $\bar{\omega}$ defined below in lemma \ref{lemma threshold}.  Here the Ramsey plan features zero transfers and the allocation corresponds to a representative agent economy where the Ramsey planner can only set labor taxes and trade assets with the household.
...
For a Pareto weight on the productive agent above a threshold $\bar{\omega}=n\left(\frac{1+\gamma}{\gamma}\right)$  using transfers is just too costly for the Ramsey planner. Positive  transfers  entail subsidizing the unproductive type 2 agents
whose consumption he values too little. Besides making the Ramsey plan in the two-type of agents economy identical to the representative agent economy with no lump sum transfers, lemma \ref{lemma threshold} also justifies our normalization where we interpret $-B_t$ as public debt. We characterize the dynamics of debt and taxes for this case and then discuss what happens when the Pareto weight of the productive agent is low. 
