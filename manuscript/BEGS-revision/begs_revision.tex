...
%% Files included:                                                     %
%% "/document/NonbalancedJune1.tex", Document, 124302, 6/18/2004, 0:45:10, ""%
...
In section \ref{Sec: quasilinear}, % with utility linear in consumption,
...
role. Our corollary \ref{corr assets dont matter} and corollary \ref{corr: B does not matter}
...
We will impose debt limits on asset profiles. These can be either be natural debt limits which correspond to the maximum debt that an agent can repay almost surely or adhoc ones that are tighter.  
...
{\color{blue} In this section we elaborate more on the quote from \citep{newcomb1865critical},  that it is not total debt but who owns it that matters for welfare. More specifically, given an initial distribution of assets, the maximum welfare that the Ramsey planner can achieve only depends on  \emph{net assets}, i.e., assets relative to those of some benchmark agent. 

\begin{theorem}
\label{thm relative assets}
For any pair $B_{-1}^{\prime },B_{-1}^{\prime
\prime }$, there are asset profiles $\left\{ b_{i,-1}^{\prime }\right\} _{i}$ and $%
\left\{ b_{i,-1}^{\prime \prime }\right\} _{i}$ such that
value of \eqref{govmt objective} at the optimal allocations starting from  $\left( \left\{ b_{i,-1}^{\prime }\right\}
_{i},B_{-1}^{\prime }\right) $ and  from  $\left( \left\{ b_{i,-1}^{\prime
\prime }\right\} _{i},B_{-1}^{\prime \prime }\right) $ are the same.
\end{theorem}
We relegate the proof to the appendix \ref{appndx: thm relative assets}. }To see the intuition for the result, imagine an increase an
...
repay $B_{-1}^{\prime }$. Since dead weight losses are convex in $\tau ,$
...
{\color{blue} We can strengthen theorem \ref{thm relative assets} for natural debt limits. The irrelevance of gross position holds not only for the initial distribution of assets but for distributions at any history associated with a competitive equilibrium. \footnote{Appendix \ref{appndx: credit limits and RE} shows how ad-hoc debt limits in the form of  exogenous history-contingent bounds for each agent can in fact expand the set of allocations that can be implemented as competitive equilibria relative to those feasible with natural debt limits.} 

\begin{corollary}
\label{corr assets dont matter} Given $\left( \left \{ b_{i,-1}\right \}
_{i},B_{-1}\right) $, let $\left \{ \left \{ c_{i,t},l_{i,t},b_{i,t}\right \} _{i},B_{t},R_{t}\right \} _{t} $ and $\left \{ \tau _{t},T_{t}\right
\} _{t}$ be a competitive equilibrium with bounded net assets. For any bounded sequences $%
...
_{-1}\right) $.
\end{corollary}
We relegate the proof to appendix \ref{appndx: corr assets dont matter}. Note that the result in Corollary \ref{corr assets dont matter} applies to \emph{any} competitive equilibrim and not just the optimal one as in Theorem \ref{thm relative assets}. In the spirit of \citet{Barro1974}, we interpret Theorem \ref{thm relative assets} and Corollary \ref{corr assets dont matter} as  asserting a type of  Ricardian equivalence. Similar results  hold in more general environments. For example, we could allow agents to trade all conceivable  Arrow securities and still show that  equilibrium allocations depend only on agents' net assets positions. Later in section  \ref{Sec: more general economies} we will use the insights from Corollary \ref{corr assets dont matter} to formulate the optimal policy problem concisely.}
...
Substituting these into the  budget constraint \eqref{agent bc affine} gives %, the implementability constraints become
...
and get 
 ...
 \end{equation}
 {\color{blue}
 Equation \eqref{implementability constraint ql} presents a recursive version of the implementability constraints appropriate for the quasilinear setting. Note from equation \eqref{implementability constraint ql} that only relative assets, $b_{1,t}-b_{2,t}$ matter, so we adopt a normalization where we set the assets $b_{2,t}$ of the unproductive agent  always to
zero. This makes the asset market clearing condition imply  that   $nb_{1,t} = -B_t$ and also implies that  transfers  $T_t=c_{2,t}$. }Next we make
\begin{assumption}
...
\noindent Let $(\omega,n) \in [0,1] \times [0,1]$ be the Pareto weight and mass assigned to the  productive type $1$ agent and $V(B\_)$ be the value of the Ramsey plan for initial government assets $B\_$.
...
\end{subequations}   
{\color{blue}
Given our normalization $b_{2,t}=0$, bounds on planner's asset positions $B_t$ are equivalently restrictions on net assets positions. We assume that households face asset limits that are looser than those implied by \eqref{ndl1}. }
...
%higher than a threshold $\bar{\omega}$ defined below in Lemma \ref{lemma threshold}.
...
% By explaining how it  is one ingredient into making   the Ramsey plan in an
%  economy with two types of agent identical to that for  representative agent economy with zero lump sum transfers, 
% Lemma \ref{lemma threshold} also provides a good property of the normalization $b_{2,t} \equiv 0$ that we have imposed to
% allow us to interpret $-B_t$ as public debt. 
...
%Let $(\omega,n) \in [0,1] \times [0,1]$ be the Pareto weight and mass assigned to the  productive type $1$ agent.
...
% a consequence of corollary \ref{corr: B does not matter} of corollary \ref{corr assets dont matter}.
...
time $0$ problem. {\color{blue} As asserted in Corollary \ref{corr assets dont matter}, with natural debt limits, public debt and transfers are 
not  pinned down separately within a competitive equilibrium. To acknowledge this situation,
for the rest of the paper  we assume that productivities are ordered as 
$\theta_{1,t}\geq\theta_{2,t}\ldots\geq \theta_{I,t}$ and then set $b_{I,t}=0$. Doing so 
allows us to interpret $-B_t=\sum_{i<I}n_ib_{i,t}$ as ``public debt''.  Limits on $\sum_{i<I}n_ib_{i,t}$ now  become  counterparts  to what would be  debt limits in a representative agent economy.} 
...
Normalizing the assets of the unproductive agent to zero %\footnote{Which corollary \ref{corr assets dont matter} again  tells us amounts only to  a normalization.}
...
% corollary \ref{corr assets dont matter} provides  a useful way to compare our results with
...
the second and third questions posed in our first paragraph of this paper are interesting only in settings in which
 government debt is a state variable.  The same fiscal-hedging motive  that  in complete markets economies % serves through
 %ample state-contincies to 
 eradicates
...
and constraints (\ref{eqn:feasiblity}), (\ref{govmt bc affine})\ and (\ref%
...
The introduction of the ad-hoc debt limits leaves unaltered the conclusions of  Theorem \ref{thm relative assets} and
...
_{i}-b_{i,t}\right\} .$ Define $\hat{b}_{i,t}\equiv b_{i,t}+\Delta _{t}$ \ for all $t\geq 0$ and $\hat{b}_{i,-1}=b_{-1}.$ By Corollary \ref{corr assets dont matter}, $\left\{ c_{i,t},l_{i,t},\hat{b}%
...
higher welfare in the economy with ad-hoc borrowing limits. For this example, we impose $g_t=0$, $P(s_t)=1$, $\theta_{i,t}=\theta_i$ and work with $I=2$.  Further the utility function over consumption and labor supply $U(c,l)$ is separable in the arguments and satisfies the Inada conditions. Slightly different from section \ref{Sec: environment}, we write the budget constraints of the agents in with prices $q_t$ rather than returns $ R_{t}$, As will be clear, this modification turns out to be convenient for this simple example. Thus the budget constraints \ref{agent bc affine} are modified to,
...
\subsection{Proof of Theorem \protect\ref{thm relative assets}}
\label {appndx: thm relative assets}
\begin{proof}

Let $W\left(\left \{b_{i,-1}\right\}_i,B_{-1}\right)$ be the welfare associated with initial assets $\{b_{i,-1}\}_i,B_{-1}$. Pick $\left(B_{-1}',\left\{ b_{i,-1}'\right\} _{i}\right)$ and  $\left(B_{-1}'', \left\{ b_{i,-1}''\right\} _{i}\right)$ such that for all $i$ we have,

\begin{equation}
\label{same relative asssets}
b'_{i,-1}-b'_{1,-1}=b''_{i}-b''_{1,-1}.
\end{equation}


WLOG suppose $W\left(\left\{b'_{i,-1}\right\}_i,B'_{-1}\right)>W\left(\left \{b''_{i,-1}\right\}_i,B''_{-1}\right)$. Let $\{\tau^*_t,T^*_t\}_t$ be the optimal policy and $\{c^*_{i,t},l^*_{i,t},b^*_{i,t}\}_{i,t},\{B^*_t\}_t,\{R^*_t\}$ be the optimal competitive allocation that attains  $W\left(\left\{b'_{i,-1}\right\}_i,B'_{-1}\right)$. 

\noindent Consider the following policy $\hat{T}_0=T^*_0+R^*_0\left(b'_{1,-1}-b''_{1,-1}\right)$ and $\hat{T}_t=T^*_t \quad \forall t\geq 1$ and $\hat{\tau}_t=\tau^*_t$. We show that  $\{c^*_{i,t},l^*_{i,t},b^*_{i,t}\}_{i,t},\{B^*_t\}_t,\{R^*_t\}$  is a feasible competitive allocation for initial conditions $\left(\left\{b''_{i,-1}\right\}_i,B''_{-1}\right)$. For this to be true we only need to verify the budget constraints of the agents at time $t=0$. For any $i$, 

\[c^*_{i,0}+b^*_{i,0}=(1-\tau^*_0)\theta_{i,0}l^*_{i,0}+
T^*_0+R^*_0 b'_{i,-1},\]
follows from the fact that $(c^*_{i,0},l^*_{i,0},b^*_{i,0})$, $R^*_0$ are a part of a competitve equilibria given $\tau^*_0$ and $T^*_0$. Substituting the definition of $\hat{T}_0$, we get 

\[c^*_{i,0}+b^*_{i,0}=(1-\tau^*_0)\theta_{i,0}l^*_{i,0}+
\hat{T}_0-R^*_0\left(b'_{1,-1}-b''_{1,-1}\right)+R^*_0 b'_{i,-1}\]

or 

\[c^*_{i,0}+b^*_{i,0}=(1-\tau^*_0)\theta_{i,0}l^*_{i,0}+
\hat T_0 + R^*_0\left(b'_{i,-1}-b'_{1,-1}+b''_{1,-1}\right)\]

Condition \eqref{same relative asssets} implies,

\[c^*_{i,0}+b^*_{i,0}=(1-\tau^*_0)\theta_{i,0}l^*_{i,0}+
\hat T_0 + R^*_0b''_{i,-1}\]

Since the budget constraints at $t=0$ are satisfied for all agents the allocation $\{c^*_{i,t},l^*_{i,t},b^*_{i,t}\}_{i,t},\{B^*_t\}_t,\{R^*_t\}$  is a feasible competitive equilibrium allocation for initial conditions $\left(\left\{b''_{i,-1}\right\}_i,B''_{-1}\right)$ given the alternative policy $\{\hat \tau_ t,\hat{T}_t\}$\footnote{The only other place $\hat T_0$ appears is the government's budget constraint. But since we have verified the agents' budget constraints and the $\{c^*_{i,0},l^*_{i,0}\}$ satisfy the resource constraint, Walras law implies the government’s budget constraint is also satisfied} and  
\[W\left(\left \{b'_{i,-1}\right\}_i,B'_{-1}\right)=\mathbb{E}_{0}\sum_{i=1}^{I}\omega_{i}
\sum_{t=0}^{\infty}\beta^{t}U_{t}^{i}\left(c^*_{i,t},l^*_{i,t}\right)>W\left(\left \{b''_{i,-1}\right\}_i,B''_{-1}\right)\]

Thus we have a contradiction that $W\left(\left \{b''_{i,-1}\right\}_i,B''_{-1}\right)$ is the maximum welfare.


\end{proof}

\subsection{Proof of Corollary \ref{corr assets dont matter}}
\label{appndx: corr assets dont matter}
 \begin{proof}
 Let
 \begin{equation}
...
\begin{equation}
\begin{split}
	\barC &= \sum_s\left\{\frac{{b^{*}}}{\beta\left[I'(\mu^*)+\frac{\partial b}{\partial\mu}\right]}\left(\pi(s) - \frac{\pi(s)P^*(s)}{\EE{P^*}^2}\right)( P(s)-P^*(s))\right\}\\
...
\end{equation*}%
Multiply by $\beta ^{t}\pi \left( s^{t}\right) U_{1}(s^{t})$ to get:%
\begin{eqnarray*}
\beta ^{t}\pi \left( s^{t}\right) U_{1}(s^{t})\mathbf{p}(s^{t})\mathbf{h}%
\left( s^{t}\right)  &=&\beta ^{t}\pi \left( s^{t}\right)
U_{1}(s^{t})\varphi (s^{t-1})-q(s^{t})\beta ^{t}\pi \left( s^{t}\right)
U_{1}(s^{t})\varphi (s^{t}) \\
&=&\beta ^{t}\pi \left( s^{t}\right) U_{1}(s^{t})\varphi (s^{t-1})-\beta
^{t+1}\sum_{s^{t+1}>s^{t}}\pi \left( s^{t+1}\right) U_{1}\left(
...
\begin{equation*}
\sum_{t=0}^{T}\beta ^{t}\pi \left( s^{t}\right) \mathbf{U}_{x}(\mathbf{x}%
_{t})\mathbf{h}\left( s^{t}\right) =-\sum_{s^{T+1}>s^{T}}\beta ^{T+1}\pi
...
\begin{equation*}
\lim_{T\rightarrow \infty }\sum_{t=0}^{T}\beta ^{t}\pi \left( s^{t}\right)
...
\begin{description}
\item[$\protect\omega_1(1-\protect\rho)+2\protect\rho^2\protect\omega_2 > 0$
...
largest positive root of $\omega_1(1-\rho)+2\rho^2\omega_2$. Note that $%
...
% 	\sum_{i=1}^Nn_i c_i(s)+g(s) = \sum_{i=1}^Nn_i \theta_i(s) l_i(s)\\
% 	\rho_i U_{c,1}(s)-U_{c,i}(s) =0
% 	n_1\alpha_1 U_{c,1}(s) + \sum_{i=2}^N\mu_i\rho_i\left[ U_{cc,1}(s) c_1(s)+U_{c,1}(s)\right] -n_1\xi(s)  +\sum_{i=2}^N \rho_i\eta_i(s)U_{cc,1}(s) = 0\\
% 	n_i\alpha_i U_{cc,i}(s) - \mu_i\left[ U_{cc,i}(s)c_i(s)+U_{c,i}(s)\right] - n_i\xi(s) -\eta_i(s) U_{cc,i}(s) = 0\\
% 	n_1 \alpha_1 U_{l,1}(s) +\sum_{i=2}^N\mu_i\rho_i\left[U_{ll,1}(s) l_1(s) + U_{l,1}(s)\right] - \sum_{i=2}^N\frac{\rho_i\phi_i(s)}{\theta_1(s)} U_{ll,1}(s) + n_1\theta_1(s)\xi(s) = 0\\
% 	n_i\alpha_i U_{l,i}(s)  -\mu_i\left[U_{ll,i}(s)l_i(s) + U_{l,i}(s)\right] +\frac{\phi_i(s)}{\theta_i(s)} U_{ll,i}(s) + n_i\theta_i(s)\xi(s) = 0\\
...
%  		&=\mathbb{E}_0[\sum_s \pi(s) B(s) \hat{\Psi}_{t-1}\hat{\Psi}_{t-1}' B(s)']-\overline B\hat{\Psi}_{t-1}\hat{\Psi}_{t-1}'\overline B'\nonumber\\
%  		&=\sum_s \pi(s) B(s)(\Sigma_{\Psi,t-1}+\hat{\Psi}_{t-1}\hat{\Psi}_{t-1}')B(s)-\overline B\hat{\Psi}_{t-1}\hat{\Psi}_{t-1}'\overline B'\nonumber\\
%  		&=\sum_s \pi(s)\left[ B(s)\Sigma_{\Psi,t-1} B(s)' +(B(s)-\overline B)\hat{\Psi}_{t-1}\hat{\Psi}_{t-1}'(B(s)-\overline B)'\right]\nonumber\\
...
\begin{lemma}  Let $\mu(s,b\_)$ be the optimal policy function for the Lagrange multiplier $\mu(s)$.  If $P(s') > P(s'')$ then there exists a $b^*_{s',s''}$ such that for all $b < (>) \; b_{1,s',s''}$ we have $\mu(s',b\_) > (<) \;\mu(s'',b\_)$.  If $\underline b < b^*_{s',s''} < \overline b$ then $\mu(b^*_{s',s''},s') = \mu(b^*_{s',s''},s'')$.
\label{lem.order}
\end{lemma}
\begin{proof}
Suppose that $\mu(s',b\_)\leq \mu(s'',b\_)$.  Subtracting the implementability for $s''$ from the implementability constraint for $s'$ we have
...
\[
	b_1 = \min\left\{b^*_{s',s''}\right\}
\] and WLOG choose $\underline b < b_1$.  For all $b < b_1$ we have shown that $P_s > P(s')$ implies that $\mu(s,b\_) > \mu(s',b\_)$.  By decomposing $\EE \mu_{t+1}P_{t+1}$ in equation \eqref{eq.mart}, we obtain (using $\EE_t P_{t+1} = 1$)
\begin{equation}
