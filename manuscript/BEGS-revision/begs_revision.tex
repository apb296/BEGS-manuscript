...
We will proceed our characterization of the Ramsey plan in two parts. First we consider the case when the Pareto weight of the productive agent is higher than a threshold $\bar{\omega}$ defined below in Lemma \ref{lemma threshold}.  Here the Ramsey plan features zero transfers and the allocation corresponds to a representative agent economy where the planner can only set labor taxes and trade assets with the household.
...
For a Pareto weight on the productive agent above a threshold $\bar{\omega}=n\left(\frac{1+\gamma}{\gamma}\right)$  using transfers is just too costly for the planner. Positive  transfers  entail subsidizing the unproductive type 2 agents
