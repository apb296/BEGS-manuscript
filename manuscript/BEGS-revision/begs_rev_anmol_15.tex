...
we assume that productivities are ordered as  $\theta_{1,t}\geq\theta_{2,t}\ldots\geq \theta_{I,t}$ and set $b_{I,t}=0$. With this normalization we can recover canonical representative agent incomplete market sets up with linear taxes and debt a-la Chari, AMSS if government does not care about redistribution, as explained in more details in \ref{Sec: quasilinear}. This allows us to interpret $-B_t=\sum_{i<I}n_ib_{i,t}$ as
...
The  general structure in Sec 3 has two difficulties: prices are party endogenous and $I$ agents makes it less transparent how along the Ramsey plan transfers are used to redistribute and smooth distortions. For this reason in this section we focus on the economy in which both of this forces are very transparent: i.e. quasi-linear economy (so that prices are exogenous) with two agents, one of whom cannot work (so that redistribution between rich and poor is very clear). We also assume iid shocks for simplicity. 
...
%\begin{equation}
%  \label{eq-QLRA obj}
%    V(B\_)=\max_{\{c_1(s),c_2(s),l_1(s),B(s)\}_s} \sum_{s}\pi(s)\omega \left(c_{1}(s)-\frac{l_{1}(s)^{1+\gamma}}{1+\gamma}\right)+(1-\omega)c_{2,t}
%    
%    
%    \left\{c(s)-\frac{l(s)^{1+\gamma}}{1+\gamma}+\beta V(B(s)) \right\}
%\end{equation}
...
After this I would say that first we consider the case when Pareto weight is sufficiently high. The reason why it is useful is because of the Lemma that states that if Pareto weight is above a certain treshold, $T_t = 0$ for all t (I would state that lemma formally). This case naturally ties to the last paragraph of Sec 3 that justifies our normalization: we are isomorphic to a representative agent economy with no lump sum transfers. I would beat this case to a pulp a-la current section 5.1 (also highlighting similarities with canonical representative agent model, but staying within our framework, connection to AMSS, why assuming $T_t >= 0$ is neither innocuous nor sensible, etc. After that I would do Section 5.2. for pareto weight below the treshold derived in Section 5.1.
...
\end{equation}
and the natural debt limits on $b_t$. At interior solutions, the first-order conditions %for the household's problem
