...
\color{red}
What are the welfare costs of public debt? What determines whether and how quickly a government should retire its debt? How should  tax rates, transfers, and government debt respond to aggregate shocks? We study how answers to these questions depend on a government's limited ability
to hedge fiscal shocks and
 redistribute.
 %We
...
their productivities and holdings of a financial asset.  They  trade a single  security whose payoff potentially depends on aggregate shocks. The planner adjusts tax rates, transfers, and asset purchases in response to aggregate shocks.
%These instruments have different welfare consequences.
Depending on how returns on the asset comove with aggregate shocks, a distribution of assets gives rise to payment flows across  agents. These require the government  to adjust labor taxes and transfers  to achieve its distributive and financing goals. While labor taxes distort  labor supplies, fluctuations in  transfers also lower welfare.
 A decrease in transfers in response to adverse aggregate shocks disproportionately affects agents who have low present values of earnings.


This paper disentangles the contending forces that arise from limits to redistribution and incompleteness of markets by  first building up analytical results in a simplified setup with quasilinear preferences.   We  validate findings from that simple
...
We impose debt limits on asset profiles.\footnote{\color{blue}For households, we shall impose natural debt limits that  depend
on the  tax policy. An alternative is to impose ad-hoc debt limits in the form of  exogenous history-contingent bounds
for each agent. Appendix \ref{appndx: credit limits and RE} discusses how restricting attention to natural debt limits for the households shrinks the set of allocations that can
be implemented as competitive equilibria. Owing to a Ricardian property, limits on gross government holdings are irrelevant (see section \ref{sec:Ricardian101}).\color{black}}
 \end{remark}
...
The ergodic joint distribution is  determined by:
a) the government's  ability to hedge aggregate shocks  by taking advantage of fluctuations in  the return on the single  asset;
and b) the government's preferences about  redistribution.

\textbf{Anmol XXXXX: the next sentence and paragraph needs work.  The rest of the paragraph could too.}
In particular, at times when net-of interest deficit is high; if the return on the asset is low, the government accumulates debt and in the other case when returns on the asset are high, it accumulates assets.
...
higher in economies in which the magnitude of this comovement between net-of interest deficits and returns on assets is larger. Governments that want more redistribution eventually issue more debt.
...
Quasilinear preference:$u(c,l)=c-\frac{l^{1+\gamma}}{1+\gamma}$
\end{assumption}
...
We shall show that Ramsey  allocations for this economy are informative about Ramsey allocations
for  multiple-agent economies with transfers but with   Pareto weights that imply that the welfare costs of  transfers are high.
 \textbf{Anmol XXXXX: in the previous sentence, do you want costs of fluctuating transfers or costs of transfers?} The results also explain how market incompleteness, as captured by the structure of $P(s)$, impedes  tax-smoothing  in the ergodic distribution. This analysis
 extends and unifies  results from the complete markets economy of \cite{LucasJr.1983} and the risk-free-bond-only economy of  \citep{Aiyagari2002}.
\color{black}
...
This economy differs from the \citet{Aiyagari2002} economy  in two ways:  first, the single  asset  traded is possibly risky  instead of  risk-free;
and second, the government  is prohibited
from ever using transfers.\footnote{\citeauthor{Aiyagari2002} allow nonnegative  transfers.} As we shall see, both of these features
contribute to modifying  the \citeauthor{Aiyagari2002}  result that in the long run the tax rate on labor is zero.
...
\end{equation}
and the debt limits $  - \overline B \geq b_t \leq  - \underline B$. \textbf{Anmol: Please check the preceding.}
We  remind the reader of  the convention  that we set  $R_{0}=\beta^{-1} P(s_0)$.
At interior solutions, the first-order conditions for the household's problem for $l$ and $b$ imply that
\begin{eqnarray*}
l_t^\gamma & = &(1-\tau_t) \theta \cr
R_t & = & \frac{p_t}{\beta}
\end{eqnarray*}
where we are again using the normalization $E_{t-1} p_t = 1$.
Using these equations % first-order conditions for the household's  optimal labor and savings decisions
to express  for the tax rate and return
 as functions of $c$ and $l$, the implementability constraints become
 \begin{equation} \label{rep agent QL seq implementability} c_t + b_t = l_t^{1+\gamma} + \frac{p_t}{\beta} b_{t-1}, \quad t \geq 0
 \end{equation}
 Equation \eqref{rep agent QL seq implementability} is a recursive version of the implementability constraints \eqref{eq imp sum t=1} appropriate for the present setting.
  %
%\begin{equation}
...
\end{subequations}
\noindent Given some initial assets $b_{-1}=-B_{-1}$, the Ramsey planner chooses an allocation and a government
...
Theorem \ref{thm: rep agent general theorem}  establishes that when the payoff vector does  not belong to $\mathcal{P}^*$, the support of the invariant distribution of assets is wide in the sense that almost all
asset sequences recurrently revisit  small neighborhoods of  any arbitrary lower and upper bounds on government assets.
Because the labor tax rate is decreasing in government assets, it varies likewise. These outcomes contrast sharply with those   in a corresponding complete market benchmark
...
With more structure on the payoff vector, theorem \ref{thm: rep agent general theorem} shows that there is an  inward drift to government assets:
...
To acquire more insights about the invariant distribution, in  theorem \ref{thm: rep agent linear policies} we linearize the law of motion for the evolution of government assets with respect
to both government assets and payoffs. We carefully  choose the point about which we take a linear  approximation, namely,
...
After government assets have converged to $B^*$ when $P(s)\in\mathcal{P}^*$, the government perfectly hedges fluctuations in
...
\end{theorem}
\textbf{Anmol :we need to complete the following sentence.}  We relegate the proof to XXXXXX.


Recall that when the payoff vector is $P^*(s)$ and the government assets are $B^*$, it can keep the tax rate constant and perfectly  hedge fluctuations in its net-of-interest deficit
by using total
income $P^*(s)B^*$ from the government portfolio.  When $P(s) \not \in \mathcal{P}^*$, the incompleteness of markets prevents complete hedging, so  shocks are imperfectly and perpetually
...
the section \ref{Sec: rep agent} representative agent economy  a second agent who has zero productivity. We  require that  the unproductive agent's consumption be nonnegative, a cheap way of inducing curvature in the Ramsey planner's indirect utility function. \textbf{Anmol XXXXXX:
I added the previous dependent clause to the last sentence.}
Given the freedom granted us by the Ricardian equivalence result discussed in section \ref{sec:Ricardian101},
...
and the present restriction $c_{2,t} \geq 0$.
%that we use to induce curvature in the Ramsey planner's indirect utility function generate the same forces.
...
\begin{enumerate}
 \item For $\omega\geq n \left(\frac{1+\gamma}{\gamma}\right)$,  $T_t=0$.  The optimal policy is same as in the representative agent economy studied in
...
Adding curvature to utility from consumption  makes the equlibrium return  endogenous even for a risk-free bond having a payoff vector $P(s)=1$.
...
\color{blue}\textbf{Tom: do some further editing here. }
To make progress, we first present a a pair of Bellman equations that express a recursive representation of the Ramsey problem as formulated in section \ref{sec: Ramsey Plan} for general preferences. We use this formulation to construct a class of economies that  asymptotically can feature complete hedging of aggregate shocks. Then in section \ref{sec: numerical results}, we use approximate solutions  of  these Bellman equations  to
 study Ramsey allocations for economies with general preferences, several types of  agents, and several  shock structures.
...
\subsection{Recursive representation of a Ramsey plans}\label{sec: two bellman equation}
...
A tell-tale sign of the time consistency of the optimal  plan is that    (\ref{eq:BM2_Bonds_cons}) is absent from the
time $0$ problem.
