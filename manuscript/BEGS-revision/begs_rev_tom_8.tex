...
p_tU_{c,t-1}^{i}}{\beta \mathbb{E}_{t-1}p_tU_{c,t}^{i}}b_{i,t-1}\ \forall i \geq 2 %
i,t \geq 1,  \label{affine implementability a}
\end{equation}
...
p_0 \beta^{-1}b_{i,t-1}\ \forall i \geq 2.
\end{equation}
...
\color{blue} \textbf{Anmol: I edited the following two paragraphs.}
...
the welfare  costs of transfers be  low enough that it can  keep the tax rate constant. The propensity  of the government to accumulate assets so that
 eventually it can use  transfers and not labor taxes  is reminiscent of \citep{Aiyagari2002}. There, with a representative agent and non-negativity constraints on transfers, the planner
...
y_{i,t}& =&y(\epsilon_t|z_{i,t},\Gamma_{t-1},\sigma_{\epsilon}) \cr
Y_{t} & =& Y(\epsilon_t|\Gamma_{t-1},\sigma_{\epsilon}) \cr
\Gamma_{t} & = & \Gamma(\epsilon_t|\Gamma_{t-1},\epsilon_t)
\end{eqnarray}
...
%respectively that solve a collection of the individual optimality equations as in \eqref{individual const} and aggregate market clearing conditions as in \eqref{aggregate const}.
	...
	\label{aggregate const}
		\int G(y_{i,t},Y,\epsilon_t|z_{i,t-1},\sigma_\epsilon) d\Gamma_{t-1}(z_{i,t-1})= 0	.
	\end{equation}
...
generate a sequence of approximations to the system of equations \eqref{eqns:Ramsey}. We generate the $t$th outcome along a sample path
by drawing pseudo random vectors $\epsilon_t$ and applying the approximation to equations \eqref{eqns:Ramsey}  for date $t$.  
The approximation to these functions at date $t$ depends on the outcomes $\{z_{i,t-1}\}_i, \{y_{i,t-1}\}_i, \Gamma_{t-1}$ generated
...
expansion to those functions at state $\{z_{i,t-1}\}_i, \{y_{i,t-1}\}_i, \Gamma_{t-1}$, an expansion
...
Let $x\_=U^i_c\_b_{i}$ and $m^i\_\propto U^i_{c}$ with $\sum_{i}m^i=1$. Note that Ricardian equivalence implies that we can normalize $\sum_{i}\frac{x^i\_}{U^i_c\_}=0$. Thus, the essential dimension of the state variables that describe the alternative Bellman equation is also $2N-2$ as in problem \eqref{eq:BM2} in section \ref{Sec: Bellman equations}. The reason we do this is to exploit the symmetry across agents that is useful in computing the derivatives.
...
\rho^i(s):&&	\alpha(s) = m^i(s) U^i_c(s)P(s)\\
\psi^i_{-}:&&	\gamma_-=  m^i_-\EE_- P U^i_c\\
