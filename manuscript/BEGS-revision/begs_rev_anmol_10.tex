...
in an economy with heterogeneous agents, aggregate uncertainty, and a single asset with a possibly risky payoff.
 \color{blue} Limits to redistribution coming from incomplete  tax instruments and limits to hedging coming from incomplete  asset markets affect optimal policies. 
...
All agents trade a single  security whose payoff can potentially depend on aggregate shocks.
...
\color{blue}
 ...
 Figure \ref{fig:affine_taxes} shows that an affine structure better approximates the US tax-transfer system than just proportional labor taxes. \color{black}
{
  \begin{figure}
    \centering
...
The appendix describes how we apply a method of \cite{Evans2014} to approximate the Ramsey plan for the $N$ type of agents economy of section \ref{sec: numerical results}. To focus on essential steps, we take a special case where productivities
...
\item A distribution $\Gamma_{t-1}$ over $z_{i,t-1}\in \mathcal{R}^{n_z}$. This can be a measure with $N$ mass points. For a $N$ type case with fixed masses of agents and no idiosyncratic risk, $\Gamma_{t-1}$ has no more information than is in the set $\{z_{i,t-1}\}_i$. However the algorithm applies for more general economies with idiosyncratic risk where the notation $\Gamma_{t-1}$  helps.

\end{itemize}

\noindent A {\em Ramsey plan} can be represented as a set of functions $\left(y,Y,\Gamma\right)$ taking  the recursive forms 
...
y_{i,t}& =&y_i(\epsilon_t;z_{i,t-1},\Gamma_{t-1},\sigma_{\epsilon}), \quad i=1,\ldots N \cr
...
organize solutions to the pertinent implementability and optimality restrictions associated with a Ramsey plan.  
...
\begin{enumerate}
\item Given some $\Gamma_{t-1}$, compute the individual and aggregate choice variables in a limiting economy with $\sigma_{\epsilon}=0$. For our problem, our choice of state variables ensures that in this limiting economy $\Gamma_{t}=\Gamma_{t-1}$.\footnote{Extensions to more general environments where there does not exist such steady states or where $\Gamma_t$ follows a deterministic path in the non stochastic limit can be found in \cite{Evans2014}.} The allocation in the limiting economy is set of values $\left(\{\bar{y}_i,\}_i,\bar{Y}\right)$ that solve\eqref{individual const} and \eqref{aggregate const} at $\sigma_\epsilon=0$, which gives us a set of non-linear equations 
\begin{equation}
...
for a complete markets economy  with initial condition $\Gamma_{t-1}$. This follows partly from the fact that as the variance of the shocks approaches zero, a risk free bond is trivially sufficient for complete spanning. 

	
	\item Next construct a truncated Taylor series approximation to the functions $y, Y$ appearing in \eqref{eqns:Ramsey} around the steady state $\bar{y}(\Gamma_{t-1})$ and $\bar{Y}(\Gamma_{t-1})$ obtained in the previous step; this yields   approximations
...
\item \color{blue}Repeat with $\Gamma_{t}$\color{black}

\end{enumerate}

The key feature of this algorithm is how the point of approximation and hence the derivatives that capture how agents respond to aggregate shocks vary along the history. This property is particularly attractive for problems where the mean of ergodic steady distribution can be sufficiently far away from the initial conditions and  the convergence to the ergodic distribution is slow. Since it is a perturbation approach, handling a high dimension $z_{i,t-1}$ is much more tractable than other methods that solve for the policy rules using projection methods using finite order polynomials as basis functions as in \cite{Judd2011}


\subsubsection{Application to our problem}
For our problem we list $\left(y_{i},Y,z_{i}\right)$ 
\begin{itemize}
\item Individual states $z_i= (m_i,\mu_i)$ and the implied distribution $\Gamma$ over $z= (m,\mu)\in \mathbb R^2$ with $N$ mass points,
\item individual choice variables $y_i = (m_i,\mu_i,c_i,l_i,\phi_i,x_{i}\_,\rho_i,,\psi_{i}\_)$, and
\item aggregate choice variables		$Y = (\tau_l,T,\alpha,\xi,\gamma_-)$,
\end{itemize}

The objects listed above are defined now using modifications of the Bellman equation for $t\geq 1$ as spelled out in problem \eqref{eq:BM2} in section \ref{sec: two bellman equation}.  The changes are so that we can adapt the algorithm easily.
...
Satisfying equation \eqref{m def} is analogous to imposing the definition of $m^i(s)$ while equation \eqref{mod asset pricing equation} imposes that all agents face the same competitive price of the asset. In particular finding a $\gamma\_$ that satisfies \eqref{mod asset pricing equation} implies that for any $i,j$,
...
The choice of system $\left(F,G\right)$ for us corresponds to the FOCs of problem \eqref{mod bellman equation} and equations \eqref{mod imp} - \eqref{normalization m}. The solutions to $\left(F,G\right)$ in our case have multiple recursive representations in particular with respect to what constitutes as the set of sufficient state variables.   
	\begin{itemize}
