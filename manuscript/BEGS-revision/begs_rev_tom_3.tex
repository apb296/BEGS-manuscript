...
\section{Quasi linear economy}
\label{Sec: quasilinear}
We specialize the problem described in section \ref{Sec: Bellman equations} by imposing the following assumptions that are maintained throughout  this section. 
\begin{assumption}
\label{ass iid}
...
Quasi linear preference: $u(c,l)=c-\frac{l^{1+\gamma}}{1+\gamma}$\footnote{\citet{Aiyagari2002} assume quasi-linear preferences in an important part of
their analysis.}
\end{assumption}
...
feasibility \eqref{feasibility}, market clearing for bonds \eqref{market clearing for bonds},  and debt limits for the government assets 
$\underline{B},\overline{B}$.\footnote{In some calculations, we will use a natural debt limit for the government.}
...
With some more structure on the payoffs, we show that there is an average inward drift to government assets.
More precisely, the Lagrange multipliers on the sequence of implementability constraints form a sub (or super) martingale in  regions with low (or high) debt.
...
mean reversion for government  debt. Mean reverting is particularly stark when $P(s)\in \mathcal{P}^*(s)$: here  government debt converges to a 
constant.\footnote{Thus, the limiting allocation is a particular \citeauthor{LucasJr} economy  with constant government  debt and taxes; however,
the level and sign of long-run government debt is determined by the joint properties of shocks and payoffs rather than
by the initial condition as it is in the \citeauthor{LucasJr} model.}
...
In a representative agent economy satisfying assumptions \ref{ass iid} and \ref{ass quasilinear}, in the  long run, government assets under a Ramsey plan
...
its expenditures and its net-of-interest deficit.   Whether it holds assets or owes government debt is determined
by the sign of the covariance of $P(s)$ with $g(s)$. 

The long-run tax rate is inversely related to $B^*$ and satisfies
the following limits,\[\lim_{B^*\to\underline{B}}\tau^*=\frac{\gamma}{1+\gamma}\quad \lim_{B^*\to\infty}\tau^*=-\infty\] 


We now  say some things about the invariant distribution of government debt and the tax rate more generally when $P(s) \not \in \mathcal{P}^*$.
Because in general, there is no closed form solution for law of motion of government debt, we use an approximation. 
We begin with the following orthogonal decomposition of an arbitrary $P(s)$:

\[P(s)=\hat{P}(s)+P^*(s)\] where $P^*(s)\in \mathcal{P}^*$ and $\hat{P}(s)$ is orthogonal to $g(s)$.
We expanding a Ramsey plan around the steady state of the $P^*(s)$ to obtain  
the next theorem.\footnote{\textbf{Tom and Anmol  XXXXXX: refine this important footnote.}
Formally,  $P^*(s) $ is obtained by projecting $P(s)$ on the space spanned by $\mathcal{P}^*$. 
These approximations differ from those obtained in Woodford (XXX) where the point of approximation is a deterministic steady state. 
Appendix \ref{} contains more details of the approximation method including comparing outcomes to those obtained numerically solving the same economy. 
\textbf{Tom and Anmol XXXXX: de-Davidify this by saying ``projection methods'' instead of ``numerically solving'' and so on.}} 
...
\begin{itemize}
 ...
 \item \textbf{Variance:} The coefficient of variation is 
  \[
...
\end{theorem}
Recall that when payoffs on the assets  are $P^*(s)$, the government can keep taxes constant and perfectly  fluctuations in its net-of-interest surplus with total
...
and the level of government debt. The theorem assets   how  deviations from complete spanning map into larger variances for government debt and the tax rate
...
\subsection{Heterogeneous agent economy with quasi-linear preferences\label{sec:hetquasilinear}}
...
Given the Ricardian equivalence result discussed in section \ref{Sec: ricardian eq}, 
...
In the theorem \ref{thm heterogeneous agents} two-type of agents economy, 
...
% efficiency
the costs to the Ramsey planner of using  fluctuating transfers to hedge aggregate shocks are endogenous.
The  environment is simple enough to allow us to pinpoint how these costs depend on the Pareto weights.
A ``regressive'' planner, one  who most about the productive type 1 agents,
faces high costs of using transfers. For a high $\omega$ Pareto planner, increasing transfers  means giving  resources to the unproductive type 2 agents
...
threshold $\bar{\omega}=n\left(\frac{1+\gamma}{\gamma}\right)$ sets transfers to zero always.  This makes  the allocation and Ramsey  plan
in the theorem \ref{thm heterogeneous agents} two-type of agents economy be  identical to the allocation for the representative agent economy
of theorem \ref{thm: rep agent general theorem}.\footnote{Provided that we now ignore the unproductive type 2 agent. When $\omega \geq \bar \omega$, type type 1 agent
...
To facilitate analysis, the section \ref{sec:hetquasilinear} economy is  simplified along several dimensions: there are no
curvature in the utility from consumption, lack of persistence in shocks,  and only two agents.
...
This confounds the effects of the planner's motives to redistribute and to use the level of government debt together with possible fluctuations in the 
return on debt to hedge government expenditure and productivity  shocks. 
...
section \ref{sec:hetquasilinear}  economy with quasi-linear preferences.
...
\end{enumerate}
In both cases, taxes $\tau(s)=\tau^{SS}$ are independent of the realized state.
\end{theorem}
...
Theorem \ref{thm long run forces}  shows  two main forces that determine the dynamics of taxes
and assets: fluctuations in inequality and fluctuations in the interest rates. Keeping interest rates fixed for the moment, the government can in principle adjust two
...
% the government  accumulates assets over time$,$ as in  AMSS.
 ...
 with  shocks.  Parts 1 and 2 of  proposition \ref{thm long run forces} indicate what drives those  fluctuations. Consider again the example of a
 ...
 times of increased need for government revenues, as in
  part 2 of  theorem \ref{thm long run forces}, and the steady state allocation features the government's owning debt. 
  
  One can see the parallel with the representative agent quasilinear economy studied in section \ref{Sec: quasilinear}. There, exploiting linearity allowed us to provide a sharper characterization of how co-variance of interest rates with exogenous shocks affected the sign (and level) of debt through expression \eqref{ss-debt}. In parts 1 and 2 of theorem \ref{thm long run forces}, with binary shocks,  altering the gap $P(s_H)-P(s_L)$ allows us to obtain the corresponding variation in interest rates. The reasoning and the underling forces are exactly the same.
...
\end{equation}
This linearized system has coefficients that are functions of the shock. The next theorem describes a simple numerical test that allows us to determine whether  this linear system converges to zero in probability.
...
We used Theorem \ref{thm: localstability} to verify local stability of a wide range of examples. Since the parameters space is high dimensional we relegate the comparative statics to Appendix \ref{apndx: stability} typical finding is that the steady state is stable and that convergence is slow. The rates of convergence are increasing in the covariance of interest rates and governments needs for revenue. 
...
In sections \ref{Sec: quasilinear} and \ref{Sec: more general economies} we used steady states to characterize the long-run behavior of optimal allocations and forces that guide the asymptotic level of net assets. 
...
We start with five types of agents\footnote{We report the results for $N=5$ to capture sufficient heterogeneity in wealth and earnings. Our methods allow us to solve for arbitrary number of agents and the qualitative/quantitative insights are unchanged by adding more intermediate types.} of equal measures with preferences $u(c,l)=\frac{c^{1-\sigma}}{1-\sigma}-\frac{l^{1+\gamma}}{1+\gamma}$. These agents will represent the $90\textsuperscript{th},75\textsuperscript{th},50\textsuperscript{th},25\textsuperscript{th},  \text{and } 10\textsuperscript{th}$ percentile of US wage distribution. 
