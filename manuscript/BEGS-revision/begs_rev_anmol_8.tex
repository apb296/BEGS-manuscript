...
This paper models how transfers, the tax on labor income, government debt, and its distribution across agents should
respond to aggregate shocks in the presence of incomplete market. A planner sets a lump sum transfer and a linear tax on labor income  
in an economy with heterogeneous agents, aggregate uncertainty, and markets restricted to a single asset with a possibly risky payoff. \color{red} We characterize how limits to redistribution coming from incomplete set of tax instruments and limits to hedging coming from incomplete set of asset markets affect the tax smoothing abilities of the planner.
...
the assets' assets and their skills; and the planner's desire to use fluctuations in 
the returns on the traded asset to compensate for missing state-contingent securities.  We verify these insights in a model parametrized to match stylized facts about  US data in terms of the underlying heterogeneity in income, wealth and the fluctuations in payoffs, wages distribution over business cycles. These features  yield predictions differ markedly from 
...
proportional labor taxes, lump sum transfers and asset purchases. Agents differ in
...
We model incomplete markets flexibly by allowing everybody to trade a single financial security whose payoffs can potentially depend on aggregate shocks.
...
These instruments has different welfare consequences. Depending on how returns on the asset comove with aggregate shocks, a given distribution of asset gives rise to payment flows among private agents and the government. This generates the need to adjust labor taxes and transfers both to achieve the redistributive and financing goals. While collecting revenue using labor taxes distorts labor supply, fluctuating transfers is also costly.  A decrease in transfers in response to adverse aggregate shocks disproportionately affects agents having low present values of earnings. 

The paper disentangles these contending forces
by  first building up analytical results in a simplified setup with quasilinear preferences.   We also validate the findings from that simple
...
To separate  the planner's desires to hedge fiscal shocks from its desires to redistribute,
we analyse a representative agent economy with quasilinear preferences and no transfers.
...
debt to drifts towards a ergodic set that maximizes the  government's ability to hedge fiscal shocks,  a set
that  primarily depends on how the payoffs on the asset  correlate with the fluctuations in government's net-of-interest deficits. In particular, if the  payoff  is  high when the government needs revenue to finance a higher deficits, optimal hedging requires the government to issue positive debt, and conversely if payoffs are low in such times, the government would hold positive assets. The magnitude of debt (or assets) is decreasing and the speed at which the debt converges is increasing in the magnitude of this comovement.
...
that tell  how the spread of the ergodic distribution of government  debt and the tax rate increases with how far the payoffs  are from allowing perfect fiscal hedging.
...
This comes from the fact that welfare costs of using transfers are lower for a more redistributive government. Consequently it relies more on transfers and has less cause to accumulate assets for hedging aggregate shocks.
...
both forces discovered by Barro play large roles.  But the distributive motives that we include alter optimal policy prescriptions.
...
expenses while manipulating equilibrium prices to redistribute wealth between
...
\citet{shin2006ramsey} extends the AMSS  (\citet{Aiyagari2002}) incomplete markets economy to have two risk-averse households who
...
chooses to issue debt so that households engage in precautionary
...
structures beyond the complete markets structure.  \citet{Wer07a} provides an extensive
...
cross section distribution of skills $\{\theta_{i,t}\}$, government expenditures $\{g_t\}$,  and the payoff $p_t$ on a asset.  
...
\noindent The government and the agents trade a single, possibly risky, asset.  
\color{black}
...
Specifying the asset payoff in this way is a convenient way for us to investigate  impacts of the correlation between asset returns, on the one hand,
...
agents to trade all conceivable  Arrow securities and still show that  equilibrium allocations depend only on agents' net assets positions.} We use it to reduce the dimension of the information encoded in a description of government policy suitable for a concise  formulation of a Ramsey problem.
...
With natural borrowing limits for households, first-order necessary conditions for the  consumers' problems
 are%
\begin{equation}
...
p_0 \beta^{-1}b_{i,t-1}\ \text{for\ all\ }
\end{equation}
...
This condition is inherited from the restriction that  only one  asset  is traded
...
time $0$ problem. We will use these Bellman equations to characterize properties of the optimal plan. Although in the section \ref{Sec: quasilinear} we exploit some simplifications that come with quasilinear utility, we return to this more general formulation in \ref{Sec: more general economies} and the numerical analysis for a calibrated version of the model in \ref{sec: numerical results}.
...
In particular, the government wants to  accumulate debt when the return on the asset is lower when  its net-of interest deficits are higher and {\em vice versa}.
...
for a multiple-agent economy with transfers under some particular restrictions on  Pareto weights that make costs of transfers are high. 
\color{black}
...
\end{equation}   
   \end{subequations}
%\color{black}
We have used the first order conditions for the household's  optimal labor and savings decisions to solve for the tax rate and prices as function of $c(s)$ and $l(s)$ in equation \ref{rep agent implementability constraint}. Also note that a consequence of quasi linearity, the Ramsey plan is time consistent and we can characterize it by a single Bellman equation instead of two as in  \ref{Sec: Bellman equations}
...
When the payoff vector does do not belong to $\mathcal{P}^*$, the support of the invariant distribution of debt is wide in the sense that almost all
...
\color{red}
If the planner begins with enough assets as in part 2.a of the theorem \ref{thm heterogeneous agents}, it can support an interior allocation where all fluctuations in expenditure are financed by fluctating $T_t$. Government assets are stationary and taxes are constant at a level that is independent of initial assets and the marginal costs of labor taxes is equal to marginal costs of transfers. 
For low enough initial assets, the planner eventually accumulates assets until it reaches the threshold where costs of transfers are sufficiently low that it can  keep the tax rate at a constant level. This tendency of the government to accumulate assets and eventually use transfers is reminiscent of \citep{Aiyagari2002}. There with a representative agent and non-negativity constraints on transfers, the planner 
accumulated enough assets such that it could finance shocks with zero distortionary labor taxes and use transfers in costless manner. 

With multiple agents costs of transfers depend on concerns of redistribution. A change in the Pareto weights that  makes the planner care more about the unproductive agent lowers marginal costs of collecting revenue from labor taxes from the productive agent. The planner increases the level of tax rate and consequently the threshold level of assets that is required to finance all shocks by transfers is lower.
\color{black}
...
\subsection{Numerical approximation to the Ramsey plan}\label{appndx: numerical appendix}
The appendix describes how we apply methods of \cite{Evans2014} to approximate the Ramsey plan for the N type agents economy in section \ref{sec: numerical results}. In order to focus on the essential steps we begin with slightly special case where wages 
\color{red}Tom: I cant read your remark here\color{black}
for agent $i$ are given by $\log \theta_{i,t}=\log \overline{\theta}_i+\sigma_{\epsilon} \epsilon_t$, where $\epsilon_t$ is i.i.d with mean zero and variance one.\footnote{Extending the method to more general productivity process as in \eqref{wage equation calibration} is straightforward.}
We will use the following notation and terminology for the rest of the section: 

\begin{itemize}

\item A vector of individual state variable $z_{i,{t-1}} \in \mathcal{R}^{n_z}$. 
\item A vector of individual variables $y_{i,t} \in \mathcal{R}^{n_y}$. This will include choice for $z_{i,t}$
\item A vector of aggregate variables $Y_t \in \mathcal{R}^{n_Y}$
\item A distribution $\Gamma_{t-1}$ over $z_{i,t-1}\in \mathcal{R}^{n_z}$. This can be a measure with $N$ mass points and in particular,  for a $N$ type case with fixed masses and no idiosyncratic risk, $\Gamma_{t-1}$ has no more information that the set $\{z_{i,t-1}\}_i$. However the algorithm described below applies for more general economies with idiosyncratic risk and the notation $\Gamma_{t-1}$ helps exposition. 

\end{itemize}


An equilibrium that we wish to approximate is a set of functions $y,Y,\Gamma$ s.t $y_{i,t}=y(\epsilon_t|z_{i,t},\Gamma_{t-1},\sigma_{\epsilon})$, $Y_{t}=y(\epsilon_t|\Gamma_{t-1},\sigma_{\epsilon})$ and $\Gamma_{t}=\Gamma(\epsilon_t|\Gamma_{t-1},\epsilon_t)$ respectively that solve a collection of the individual optimality equations as in \eqref{individual const} and aggregate market clearing conditions as in \eqref{aggregate const}.
...
The terms $E_{t-1}y_{i,t}$ in equation \ref{individual const} are stand in to capture a constraint that a subset of individual controls $y_{i,t}$ can be $t-1$ measurable. For the calculations in section \ref{sec: numerical results}, we construct an approximation to $\{y_t,Y_t,\Gamma_t\}_t$ for a given sequence of aggregate shocks $\epsilon^t$ and some initial $\Gamma_{-1}$.


The approximation method proceeds sequentially.

\begin{enumerate}
\item Given some $\Gamma_{t-1}$, compute the individual and aggregate variables in a limiting economy with $\sigma_{\epsilon}=0$. For our problem, our choice of state variables ensures that in this limiting economy $\Gamma_{t}=\Gamma_{t-1}$.\footnote{The extension to more general environments where there does not exist such steady states or $\Gamma_t$ follows a deterministic path in the non stochastic limit can be found in \cite{Evans2014}} This solves \eqref{individual const} and \eqref{aggregate const} at $\sigma_\epsilon=0$. We end up the a set of non-linear equations, 
\begin{equation}
...
This steady state $\bar{y},\bar{Y}$ depends on $\Gamma_{t-1}$ and has an interpretation that it recovers a complete markets economy (as the variance of the shocks go to zero, a risk free bond is trivially sufficient for complete spanning) with initial conditions $\Gamma_{t-1}$.

	
	\item Next construct a truncated Taylor approximation to the individual and aggregate variables around the steady state obtained in the previous step, $\bar{y}(\Gamma_{t-1})$ and $\bar{Y}(\Gamma_{t-1})$; this yields us an approximation
...
The main computational task involves calculating the derivatives evaluated at the steady state.  These satisfy the total differentiation of system \eqref{individual const} and \eqref{aggregate const} at the non stochastic steady state associated with $\Gamma_{t-1}$. \footnote{For large $N$,calculating these derivatives can be further simplified for a class of problems where $\frac{\partial z_{i,t}}{\partial z_{i,t-1}}(0|z_{i,t-1},\Gamma_{t-1},0)$ is independent of $z_{i,t-1}$. In our context this turns out to be an identity matrix.}
...
\psi^i_{-}:&&	\gamma_-=  m^i_-\EE_- P U^i_c\\these
