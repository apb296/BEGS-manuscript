...
In this economy, the main choice the government faces is what policy mix of tax rates, transfers and asset purchases is best suited to respond to aggregate shocks that affect its need for revenues. Each of these instruments involves a different tradeoff in terms of welfare. Firstly depending on how returns on assets co-move with aggregate shocks a given distribution of asset introduces financial flows between the private agents and the government that may differ from the what the government desires in order to minimize efficiency costs associated with labor taxes while achieving both its financing and  redistribution needs. Although unrestricted, fluctuating transfers is costly too as a decrease in transfers in response to adverse aggregate shocks disproportionately affects agents having low present values of earnings. Thus concern for redistribution interacts with limited hedging abilities that are implicit with incomplete markets. 


The paper attempts to disentangle these considerations by  building up analytical results in a setup with quasilinear preferences and then validates the findings in a more quantitative exercise where the model is calibrated to US data.
...
\textbf{Anmol XXXXXX: it seems that we can move and/or change the preceding sentence about ordering of $\theta_i$'s  anywhere we want -- here it is only used in the following footnote.  I flag
the ordering issue for you below too.}
...
This in turn explains why imposing limits on $B_t-b_{N,t}$ are comparable to debt limits in a representative agent settings. 
