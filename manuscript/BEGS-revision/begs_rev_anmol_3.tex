...
\color{red}
Substituting the optimal labor and savings decisions, we can characterize the optimal Ramsey plan using the following Bellman equation. Let $V(B\_)$ be the maximum ex-ante value the government can achieve with assets $B\_$. 
...
by the initial condition as it is in the \citeauthor{LucasJr.1983} model.}
...
Formally,  $P^* $ is obtained by projecting $P$ on the space spanned by $\mathcal{P}^*$. We then take a first order Taylor approximation to the policy rules and laws of motion for the state variables of our economy around the complete market counterparts associated with $P^*\in \mathcal{P}^*$.
These approximations differ from  where the point of approximation is a deterministic steady state. 
