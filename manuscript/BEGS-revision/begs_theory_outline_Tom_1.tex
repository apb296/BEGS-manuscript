...
\textbf{Anmol XXXXX: please clarify what ``it can be perfectly span'' means.}
In the case where it can perfectly span
we can express the long run assets \[B^*=\beta \frac{\var(g(s))}{\cov(P(s),g(s))}\]Thus, if the pays off are more in the low expenditure states, the government issues debt in the long run and vice versa. The magnitude is determined by the strength of the covariance. The level of debt determines 
the  span required to cover the fluctuations in surplus while keeping taxes \textbf{Anmol: clarify difference between tax rate and amount of tax revenues raised.}
constant. The long run tax rate is inversely related to $B^*$ with the following limits,\[\lim_{B^*\to\underline{B}}\tau^*=\frac{\gamma}{1+\gamma}\quad \lim_{B^*\to\infty}\tau^*=-\infty\] 
...
For more general payoff structures $P(s)$ we appeal to an approximation around the payoff that is perfectly aligned with $g(s)$. In particular, we obtain a orthogonal decomposition of $P(s)$ as follows,
...
\textbf{Anmol: here we need a cite to David and his method don't we?}
...
For $\|\hat{P}(s)\|$ small, the ergodic distribution of debt of the policy rules linearized around $(B^*, P^*(s))$ will have mean $B^*$ and and coefficient of variation
  ...
  \]
  The speed of convergence to the ergodic distribution described by
  \[
