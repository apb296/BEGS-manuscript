...
A distribution of assets  gives rise to  flows of asset earnings across  agents that depend on how returns on the asset comove with aggregate shocks. These
...
Net, not gross asset positions, not affect the set of allocations that can be implemented
...
on the single asset is a key intermediating object that shapes the ergodic joint distribution of the tax rate and government debt.  In our general setting, one-period utilities are concave in consumption. That makes  the return on the single asset an e equilibrium objects partly under the control of the Ramsey planner.
...
achieves complete hedging by having the appropriate constant level of government debt or assets while managing to keep both the tax rate on labor
 and transfers constant.  %\textbf{Tom  HHHHHH: mention approx method here -- maybe later.}
...
In this paper, we have adhered to the adage that technique ought to  efface itself in the service of economic analysis.
...
In section \ref{Sec: quasilinear},
 we exploit some simplifications that come with quasilinear utility, but we return to this more general formulation in sections \ref{Sec: more general economies} and %the numerical analysis for a calibrated version of the model in section
...
The results  also shed light on  how market incompleteness, as captured by the structure of $P(s)$, impedes  tax-smoothing  in a joint ergodic distribution for the tax rate, transfers, and government debt.
%This analysis
...
We alter the  \citet{Aiyagari2002} economy  in two ways:  first,  the single  asset  can  be risky; %instead of  risk-free;
...
At interior solutions, the first-order conditions %for the household's problem
...
to express   the tax rate and asset return
 as functions   $c$ and $l$ and then substituting them into the  budget constraint \eqref{eqn:recurbud1} gives %, the implementability constraints become
...
following  set %of payoffs
 indexed by a constant $B^*$  %$\mathcal{P}^*$ defined below,
...
government's net-of-interest deficit is high, then owing government debt is useful because it helps  lower the government's interest burden in the face of adverse government expenditure shocks., after government assets have converged to $B^*$, % under the case 3 that assumes that  $P(s)\in\mathcal{P}^*$,
the government perfectly hedges fluctuations in
...
%We now  turn to another  special economy, one that now features both  heterogeneous agents and  transfers.
 We now alter the
the section \ref{Sec: rep agent} % representative agent
...
\end{assumption}
 %inducing curvature in the Ramsey planner's indirect utility function. \textbf{Anmol XXXXXX:
...
%The assumption that $\theta_2=0$  allows us easily to characterize how  the Ramsey plan  depends on the Pareto weights.
...
For a Pareto weight on the productive agent above a
...
% So a Ramsey planner who assigns  a Pareto weight $\omega$ to the productive type 1 agent above a
%threshold $\bar{\omega}=n\left(\frac{1+\gamma}{\gamma}\right)$  chooses always to sets transfers to zero.
...
of theorem \ref{thm: rep agent general theorem}. % when $\omega \geq \bar \omega$,
...
%\textbf{Anmol and Tom:  I want to discuss some of the sentences in the next two paragraphs and seek clarification.}
 Things change in part 2  theorem \ref{thm heterogeneous agents} with a more progressive  $\omega<\bar{\omega}$ Ramsey planner.
Fluctuating transfers  now become a useful  tool to  hedge aggregate shocks.
 ...
 to the marginal welfare costs of transfers. Part 2.b describes a setting in which  $P(s)\not \in \mathcal{P}^*$ and government assets too low to validate the part 2.a outcome.  Here the planner eventually accumulates government  assets until they reach the threshold $\mathcal{B}(\omega)$
 %.  At this level,
...
%The section \ref{sec:hetquasilinear} economy simplifies things along several dimensions: except for the constraint $c_{2,t} \geq 0$, there is no
...
for more general specifications, we use a numerical  convergence criterion to  establish local stability of a steady state over a range of parameter values. \textbf{Anmol XXXXX: please describe the dimensions of more generality -- e.g., persistence, curvature, number of agents --
