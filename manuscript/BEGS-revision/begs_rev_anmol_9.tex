...
This paper models how transfers, a tax rate on labor income, and  the distribution of government debt across agents should
respond to aggregate shocks in the presence of incomplete markets. A planner sets a lump sum transfer and a linear tax on labor income
in an economy with heterogeneous agents, aggregate uncertainty, and markets restricted to a single asset with a possibly risky payoff.
 \color{black} Limits to redistribution coming from incomplete  tax instruments and limits to hedging coming from incomplete  asset markets affect optimal policies.
Two forces shape long-run outcomes: the planner's desire to minimize the welfare costs of fluctuating transfers, which calls for a negative correlation between
the agents' assets and their skills; and the planner's desire to use fluctuations in
...
Ramsey plans for representative agent  models.\color{black}
...
\color{black}
What are the welfare costs of public debt? What determines whether and how quickly a government should retire its debt? How should  tax rates, transfers, and government debt respond to aggregate shocks?
We answer these questions for an economy with heterogeneous agents, incomplete markets,  and a government that chooses
...
We model incomplete markets flexibly by allowing everybody to trade a single  security whose payoffs
 can potentially depend on aggregate shocks.




The main trade-offs facing the Pareto planner
arise from the interaction of two features: limits to redistribution and incompleteness of markets. The planner adjusts tax rates, transfers, and asset
purchases in response to aggregate shocks.
These instruments have different welfare consequences. Depending on how returns on the asset comove with aggregate shocks, a given distribution of asset gives rise to payment flows across  agents. Those require that the government  adjust labor taxes and transfers  to achieve its distributive and financing goals. While labor taxes distort  labor supplies, fluctuations in  transfers are  also costly in terms of welfare.
...
The paper disentangles these contending forces
by  first building up analytical results in a simplified setup with quasilinear preferences.   We  validate the findings from that simple
setup in a more general quantitative model calibrated to US data.

We exploit  a Ricardian property.
Gross asset positions  do not affect the set of feasible allocations that can be implemented
in competitive equilibria with a proportional labor tax and transfers.
This insight reduces the dimension of the state needed to keep track of a Ramsey plan.
...
that  primarily depends on how the payoffs on the asset  correlate with  fluctuations in the
 government's net-of-interest deficit. In particular, if the  payoff  is  high when the government
 needs revenue to finance a higher deficit, optimal hedging requires the government to issue positive debt, and, conversely, if the
...
With quasilinear preferences, we establish that the asymptotic level of assets is decreasing in the planner's redistributive concerns.
This comes from the fact that welfare costs of using transfers are lower for a more redistributive government.
Consequently it relies more on transfers and has less cause to accumulate assets for hedging aggregate shocks.
...
cross section distribution of skills $\{\theta_{i,t}\}$, government expenditures $\{g_t\}$,  and the payoff $p_t$ on an asset.
...
\end{equation}
The government and the agents trade a single, possibly risky, asset.
\color{black}
...
We impose debt limits.  For households, we shall impose natural debt limits that will depend
...
agents to trade all conceivable  Arrow securities and still show that  equilibrium allocations depend only on agents' net assets positions.} We use
the theorem  to reduce the dimension of the information to be
encoded in a description of government policy suitable for a concise  formulation of a Ramsey problem.
...
know who owns  the initial government debt $B_{-1}^{\prime }$. For example, suppose that agents own equal amounts. Then
...
budget constraints (\ref{agent bc affine}) yields implementability constraints at $t =0 $ 
\begin{equation}
...
\end{equation}
provided that we interpret the condition at time $t=0$ in terms of a normalization.  
\textbf{Anmol: I edited the previous sentence a little.}
For $t=0$, we normalize $R_{-1,0}=P(s_0)\beta^{-1}$ and have the following implementability constraint
\begin{equation}
c_{i,0}+b_{i,0}=-\frac{U_{l,0}^{i}}{U_{c,0}^{i}}l_{i,0}+T_{0}+
p_0 \beta^{-1}b_{i,-1}\ \forall i \geq 1.
\end{equation}

\color{red}Tom XXXX I am not sure what you want here \color{black}
...
subject to
 ...
 \label{eq imp sum t=0}
  \tilde{b}_{i,-1}p_0=\mathbb{E}_{-1}\sum^{\infty}_{k=0}\beta^{k}\left(\frac{U^i_{c,k}}{U^i_{c,t}}\right) Z^i_{k}
 \end{equation}
\begin{equation}
...
\end{equation}
\begin{equation}
\sum_{i>1}\frac{x_i(s)}{U^{i}_{c}(s)} \text{is bounded}
\end{equation}
\end{subequations}
...
\begin{equation}
U_{c,0}^{i}\left[ c_{i,0}-c_{1,0}\right] +x_{i,0}+\left( {U_{l,0}^{i}} l_{i,0}-U_{c,0}^{i}\frac{U_{l,0}^{1}}{U_{c,0}^{1}}l_{1,0}\right) = \beta^{-1}p_0U_{c,0}^{i}\tilde{b}_{i,-1} \text{ for all } i\geq 2
\end{equation}
...
time $0$ problem. We will use these Bellman equations to characterize properties of the optimal plan. In section \ref{Sec: quasilinear},
...
The ergodic joint distribution is  determined by two factors:
...
In particular, the government wants to  accumulate debt if the return on the asset is lower when  its net-of interest deficits are higher and {\em vice versa}.
...
a simplfied  economy with quasilinear household preferences and i.i.d aggregate shocks. This setting is  tractable enough to 
 isolate the  main forces that drive the outcomes summarized in the preceding paragraph.  In section \ref{Sec: more general economies},
...
for a multiple-agent economy with transfers but with   Pareto weights that make the welfare costs of transfers be high.
\color{black}
...
\end{equation}
   \end{subequations}
%\color{black}
We have used the first-order conditions for the household's  optimal labor and savings decisions to solve for the tax rate and prices as function of $c(s)$ and $l(s)$ in equation \eqref{rep agent implementability constraint}. A consequence of quasilinearity is that the Ramsey plan is time consistent and that  we can characterize it by one Bellman equation instead of two as in section \ref{Sec: Bellman equations}


\subsubsection{Results}

 Theorems \ref{thm: rep agent general theorem} and \ref{thm: rep agent linear policies} assert our main results about this representative agent economy.
...
When the payoff vector does  not belong to $\mathcal{P}^*$, the support of the invariant distribution of debt is wide in the sense that almost all
debt sequences recurrently revisit  small neighborhoods of  any arbitrary lower and upper bounds on government debt.
Because labor tax rate is increasing in government debt, it varies likewise. These outcomes contrast sharply with those   in a corresponding complete market benchmark
...
the next theorem.\footnote{\color{black}
...
transfers are useful to hedge aggregate shocks as the costs are lower. However the costs of transfers depend jointly on Pareto weights and the distribution of assets or the government assets $B_t$ under our normalization that the unproductive agent has zero assets.

\color{blue} \textbf{Tom XXXX: I re-edited a few lines.}
If the government begins with enough assets as in part 2.a of  theorem \ref{thm heterogeneous agents}, the planner
 can support an interior allocation in which all fluctuations in government expenditures are financed by fluctuating $T_t$. Government assets are stationary and the tax rate is  constant at a level that is independent of initial assets;  marginal welfare costs of labor taxes are equated 
 to marginal welfare costs of transfers.
For low enough initial government  assets, the planner eventually accumulates government  assets until they reach a level big enough to make
the welfare  costs of transfers be  low enough that it can  keep the tax rate constant. The propensity  of the government to accumulate assets so that
 eventually it can use  transfers is reminiscent of \citep{Aiyagari2002}. There, with a representative agent and non-negativity constraints on transfers, the planner
...
to dispose of excess earnings on its asset holdings.  

With multiple agents, welfare costs of fluctuating transfers depend on concerns for redistribution.
  Pareto weights that  make the planner care more about the unproductive agent lower the marginal welfare costs of collecting revenue from labor taxes paid by the productive agent. The planner increases the labor tax rate, making  the threshold level of assets that is required to finance all shocks by transfers be lower.
\color{black}
...
to be   consistent with the requirement that the resulting optimal law of motion for the state variables satisfy the  definition \ref{defn: steady state}.\footnote{Appendix \ref{apndx: numerical methods} discusses  second-order conditions that ensure these policies are optimal.}
...
that affect the net-of-interest government deficit. We return to this feature in section \ref{sec: numerical results} where we study low frequency components of government debt.
...
The appendix describes how we apply a method of \cite{Evans2014} to approximate the Ramsey plan for the $N$ type of agents economy of section \ref{sec: numerical results}. To focus on the essential steps, we begin with slightly special case where wages
...
\item Vectors $z_{i,{t-1}} \in \mathcal{R}^{n_z}$ of individual state variables for $i =1, \ldots, N$.
\item Vectors $y_{i,t} \in \mathcal{R}^{n_y}$ of individual choice variables for $i =1, \ldots, N$; the vector $y_{i,t}$ often includes components of $z_{i,t}$.
\item A vector $Y_t \in \mathcal{R}^{n_Y}$ of   policy variables chosen by a Ramsey planner
\item A distribution $\Gamma_{t-1}$ over $z_{i,t-1}\in \mathcal{R}^{n_z}$. This can be a measure with $N$ mass points and in particular,  for a $N$ type case with fixed masses and no idiosyncratic risk, $\Gamma_{t-1}$ has no more information that the set $\{z_{i,t-1}\}_i$. However the algorithm applies for more general economies with idiosyncratic risk, and the notation $\Gamma_{t-1}$ helps exposition.

\end{itemize}

A {\em Ramsey plan} can be represented as a set of functions $y,Y,\Gamma$ taking  the recursive forms 
...
y_{i,t}& =&y(\epsilon_t;z_{i,t-1},\Gamma_{t-1},\sigma_{\epsilon}) \cr
...
These functions are constructed by finding and appropriately
organizing solutions to the pertinent implementability and optimality restrictions associated with a Ramsey plan.  
For our problem, these restrictions can be expressed in the following forms:
%respectively that solve a collection of the individual optimality equations as in \eqref{individual const} and aggregate market clearing conditions as in \eqref{aggregate const}.
	...
	\end{equation}
	which must hold for all $z_{i,t-1}$ such that $\Gamma_{t-1}(z_{i,t-1})>0$
		\begin{equation}
	\label{aggregate const}
		\int G(y_{i,t},Y,\epsilon_t;z_{i,t-1},\sigma_\epsilon) d\Gamma_{t-1}= 0	.
	\end{equation}
	The terms $E_{t-1}y_{i,t}$ in equation \eqref{individual const}  capture  constraints requiring  that  subset of individual decision variables $y_{i,t}$ must be $t-1$ measurable. 
...
generate a sequence of approximations to the system of equations \eqref{eqns:Ramsey}. We generate the $ttextsuperscript{th}$ outcome along a sample path
by drawing pseudo random vectors $\epsilon_t$ and applying the approximation to equations \eqref{eqns:Ramsey}  for date $t$.  
...
expansion (i.e around $\sigma_{\epsilon}=0$) to those functions at state $\{z_{i,t-1}\}_i, \Gamma_{t-1}$, an expansion
 that exploits substantial properties of the economics associated with the limiting zero-noise economy at that state.  
 Thus, to approximate sample paths drawn from the recursive system \eqref{eqns:Ramsey}, we use a small noise approximation that computes a sequence of Taylor series approximations around a sequence of points generated
...
Here are the steps used to generate a sample outcome at date $t$. 

\begin{enumerate}
\item Given some $\Gamma_{t-1}$, compute the individual and aggregate variables in a limiting economy with $\sigma_{\epsilon}=0$. For our problem, our choice of state variables ensures that in this limiting economy $\Gamma_{t}=\Gamma_{t-1}$.\footnote{The extension to more general environments where there does not exist such steady states or $\Gamma_t$ follows a deterministic path in the non stochastic limit can be found in \cite{Evans2014}.} This solves \eqref{individual const} and \eqref{aggregate const} at $\sigma_\epsilon=0$. We obtain a set of non-linear equations 
\begin{equation}
	\label{individual const ss}
			F(\bar y_{i}, \bar{y}_i, \bar{Y}, \bar{y} ,0; \bar{z}_{i,t-1},0) = 0  \quad \forall i
	\end{equation}	
	\begin{equation}
	\label{aggregate const ss}
		\int G(\bar{y},\bar{Y},0;z_{i,t-1},\sigma_\epsilon) d\Gamma_{t-1}= 0	
	\end{equation}
that we solve for a steady state $\bar{y},\bar{Y}$ that depends on $\Gamma_{t-1}$. This steady state would be the outcome
for a complete markets economy  with initial conditions $\Gamma_{t-1}$. This follows partly from the fact that as the variance of the shocks go to zero, a risk free bond is trivially sufficient for complete spanning. 

	
	\item Next construct a truncated Taylor approximation to the functions $y, Y$ appearing in \eqref{eqns:Ramsey} around the steady state obtained in the previous step, $\bar{y}(\Gamma_{t-1})$ and $\bar{Y}(\Gamma_{t-1})$; this yields   approximations
	\begin{align}
...
\end{align}	
	The main computational task involves calculating the derivatives evaluated at the steady state.  This involves  totally  differentiating
 system \eqref{individual const} and \eqref{aggregate const} at the non stochastic steady state associated with $\Gamma_{t-1}$. \footnote{For large $N$,calculating these derivatives can be further simplified for a class of problems where $\frac{\partial z_{i,t}}{\partial z_{i,t-1}}(0;z_{i,t-1},\Gamma_{t-1},0)$ is independent of $z_{i,t-1}$. In our context this turns out to be an identity matrix.}

\item Draw shocks $\epsilon_t$ and use the approximated policies in the previous step for each agent to obtain estimates for $y_{i,t}$ and $Y_{t}$. Remember that $z_{i,t}$ is assumed to be included in the vector $y_{i,t}$ and this yields us the next $\Gamma_t$.
\item Repeat step 1 with $\Gamma_{t}$

\end{enumerate}

The key feature of this algorithm is how the point of approximation and hence the derivatives that capture how agents respond to aggregate shocks varies along the history. This property is particularly attractive for problems where mean of ergodic steady distribution can be sufficiently far away from the initial conditions and  the convergence to the ergodic distribution is slow. Since it is a perturbation approach, handling a high dimension $z_{i,t-1}$ is much more tractable than other methods that solve for the policy rules using projection methods using finite order polynomials as basis as in \cite{Judd2011}


\subsubsection{Application to our problem}
We describe  $y_{i},Y,z_{i}$ and $F,G$ for our problem. There are two choices we make which are worth noting: firstly we express the problem \eqref{eq:BM2} in section \ref{sec: two bellman equation} in a slightly different manner. Secondly the solutions to FOC $F,G$ in our case have multiple recursive representations in particular with respect to what constitutes as the set of sufficient state variables. We make choices that turn out to be convenient for applying the perturbation method.

We modify the Bellman equation from $t\geq 1$ described in section \ref{Sec: Bellman equations}  as follows:

Let $x\_=U^i_c\_b_{i}$ and $m^i\_\propto \frac{1}{U^i_{c}}$ with $\sum_{i}m^i=1$. Note that Ricardian equivalence implies that we can normalize $\sum_{i}\frac{x^i\_}{U^i_c\_}=0$. Thus, the essential dimension of the state variables that describe the alternative Bellman equation is also $2N-2$ as in problem \eqref{eq:BM2} in section \ref{Sec: Bellman equations}. The reason we do this is to exploit the symmetry across agents that is useful in computing the derivatives.
...
\]subject to
\begin{align}
\mu^i(s):&&	\frac{P(s)U^i_c(s) x^i_{-} }{\beta\EE_- PU^i_c} = U^i_c(s)(c^i(s)-T(s)) + U^i_l(s) l^i(s) + x^i(s)\\
...
\xi(s):&&	\sum_in^i\left[a(s)\theta^il^i(s) - c^i(s)\right] =0\\
	&&\sum_i \frac{x^i(s)}{m^i(s)} = 0\\
	&&\sum_i m^i(s) = 1
\end{align}


Satidfying equation \eqref{m def} is analogous to imposing the definition of $m^i(s)$ and equation \eqref{mod asset pricing equation} imposes that all agents face the same competitive price of the asset. In particular finding a $\gamma\_$ that satisfies \eqref{mod asset pricing equation} implies that for any $i,j$,

\[\frac{m^i\_}{m^j\_}=\frac{\mathbb{E}P U_c^j}{P U^i_c}\]

Using the definition of $m^i$, we get 

\[\frac{U^i_c(s\_)}{U^j_c(s\_)}=\frac{\mathbb{E}P U_c^i}{\mathbb{E}P U_c^j}\]

Thus we see that  these two equations, namely \eqref{m def} and \eqref{mod asset pricing equation} corresponds to equations \eqref{eq:BM2_rhoprime} and \eqref{eq:BM2_Bonds_cons} of the formulation in the \ref{sec: two bellman equation}. 


The maximization is over $\{\vec c(s),\vec l(s),\vec x(s),\tau_l(s),T(s),\alpha(s),\gamma_-\}$.
%\textbf{Anmol XXXXX: let's discuss the above and below to make sure it makes sense.  E.G, the presence
%of $\gamma_-$.  }


Now we describe our second choice of what constitutes $z_{i}$. The FOC's of the problem described above can be expressed as
	\begin{itemize}
		\item  Allocations as functions of $(\vec x_-,\vec m_-,s)$ and $\vec\mu_-(\vec x_-,\vec m_-)$ with law of motion for $(\vec x_-,\vec m_-)$

		v.s
		
		\item  Allocations as  functions of $(\vec \mu_-,\vec m_-,s)$ and $\vec x_-(\vec \mu_-,\vec m_-)$ with law of motion for $(\vec \mu_-,\vec m_-)$
	\end{itemize}
We implement the second alternative because when  $\sigma_{\epsilon}=0$, for $N>2$ that there are multiple $\mu\_$ associated with an arbitrary $x\_$ that solve the FOC but technically only one survives if one takes a limit $\sigma_{\epsilon}\to 0$.  However, flipping the state to be $\mu\_$ retrieves a unique $x\_$.\footnote{We solved in both ways (using projection methods) for $N = 2$ to verify that they produce same allocation.}

%We augment the FOC with constraints that impose a subset of the controls $( x_-, \psi_-,\gamma_-)$ are independent of $s$.
%\[\mathbb{E}\]


Collecting these outcomes provides us with
\begin{itemize}
\item Individual states $z^i= (m^i,\mu^i)$ and the implied distribution $\Gamma$ over $z= (m,\mu)\in \mathbb R^2$ with $N$ mass points,
\item individual variables $y = (m,\mu,c,l,\phi,\rho,x_-,\psi_-)$, and
\item aggregate variables		$Y = (\tau_l,T,\alpha,\xi,\gamma_-)$,
\end{itemize}

for each date.\footnote{Lastly wage process like in equation $\eqref{wage equation calibration}$ are implemented by adding a auxiliary aggregate state variable $\mathcal{Q}_t$ that is a vector of length $N$ and stores the quantiles of agents 1,2 ..$N$ respectively. Its (trivial) law of motion satisfies $\mathcal{Q}_t=\mathcal{Q}_{t-1}$. This allows us to express the vector of wages as a function of the shock and the state variable $\mathcal{Q}_t$.}
